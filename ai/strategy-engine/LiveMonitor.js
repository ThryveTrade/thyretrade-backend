...
// Simple real-time monitor for Bitcoin using CoinGecko
async function monitorOneMinuteBreakout() {
  ...
  while (true) {
    try {
      ...
      if (err.response && err.response.status === 429) {
        console.error(chalk.red("Rate limit exceeded, waiting 60 seconds..."));
